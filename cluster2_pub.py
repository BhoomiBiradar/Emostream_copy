...
output_topic = "cluster2_topic"  # The unique topic for this cluster's subscribers
...
def create_cluster_publisher(input_topic, output_topic):
    ...
    print(f"Cluster 2 Publisher is listening to '{input_topic}' and publishing to '{output_topic}'...")
    
    for message in consumer:
        print(f"Cluster 2 received message: {message.value}")
        
        # Forward message to the single cluster-specific topic
        producer.send(output_topic, value=message.value)
        print(f"Cluster 2 forwarded message to '{output_topic}'")
